...
const ConfirmationModal = ({ isOpen, onClose, onConfirm, title, children }) => {
  ...
  return (
    ...
    >
      <div
        className="bg-slate-900 border border-slate-700 rounded-xl shadow-2xl p-6 w-full max-w-md mx-4"
...
const StatCard = ({ title, value, icon: IconComponent, description }) => (
  <div className="bg-gradient-to-br from-[#21312E] to-[#263834] border border-gray-700/50 rounded-xl p-5 shadow-lg hover:shadow-cyan-500/10 hover:scale-[1.02] transition-all duration-300">
    ...
    <h3 className="text-4xl font-bold text-slate-100 mt-2 bg-clip-text text-transparent bg-gradient-to-r from-teal-300 to-cyan-400">
      {value}
    </h3>
...
const Notification = ({ message, type, onDismiss }) => {
  ...
  const baseClasses =
    "mt-4 p-3 text-sm rounded-lg border flex items-center justify-between";
  const typeClasses = {
...
const BuyerDashboard = () => {
  ...
  const { user } = useAuth();
  const { tokens, loading, fetchTokens, transferToken, retireToken } =
    useBlockchain();
  ...
  useEffect(() => {
    const fetchBuyerData = async () => {
      try {
        const mockData = {
          marketPrices: [
            { type: "Solar-Powered", price: 88.5, change: "+1.2%" },
            { type: "Wind-Powered", price: 85.0, change: "-0.5%" },
  ...
  const portfolioValue = useMemo(
    () =>
      (ownedTokens.length * 85).toLocaleString("en-US", {
        style: "currency",
        currency: "USD",
      }),
    [ownedTokens]
  ...
  const handleTransferToken = async (e) => {
    ...
    if (!selectedTokenId || !transferToAddress.trim()) {
      showNotification(
        "Please select a token and enter a recipient address.",
        "error"
      );
      return;
    }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-[#1A2421] to-[#1F2C28] text-gray-300 font-sans p-8">
      ...
      >
        <p>
          Are you sure you want to retire Token #{tokenToRetire}? This action is
          irreversible and will permanently remove the credit from circulation.
        </p>
      </ConfirmationModal>
      ...
      <p className="text-md text-gray-400 mb-8">
        Manage your portfolio of green hydrogen credits.
      </p>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        <StatCard
          ...
          icon={FiCreditCard}
          description="Active credits in your wallet"
        />
        <StatCard
          title="Retired Credits"
          value={retiredTokens.length}
          icon={FiCheckCircle}
          description="Credits you have retired"
        />
        <StatCard
          title="Total Credits"
          value={ownedTokens.length + retiredTokens.length}
          icon={FiDatabase}
          description="All credits you've interacted with"
        />
        <StatCard
          title="Portfolio Value"
          value={portfolioValue}
          icon={FiDollarSign}
          description="Estimated value of owned credits"
        />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 space-y-8">
          <div className="bg-[#21312E]/50 border border-gray-700/50 rounded-xl shadow-xl">
            <div className="p-6">
              <h3 className="text-lg font-semibold text-white mb-4">
                My Credits ({ownedTokens.length})
              </h3>
              <div className="max-h-96 overflow-y-auto space-y-3 pr-2">
                {loading ? (
                  <div className="text-center p-4">Loading credits...</div>
                ) : ownedTokens.length > 0 ? (
                  ownedTokens.map((token) => (
                    <div
                      key={token.tokenId}
                      className="border border-gray-700 rounded-lg p-4 bg-slate-800/50 flex justify-between items-start"
                    >
                      ...
                      <button
                        onClick={() => initiateRetireToken(token.tokenId)}
          ...
          <div className="bg-[#21312E]/50 border border-gray-700/50 rounded-xl shadow-xl">
            <div className="p-6">
              <h3 className="text-lg font-semibold text-white mb-4">
                My Retired Credits ({retiredTokens.length})
              </h3>
              <div className="max-h-60 overflow-y-auto space-y-3 pr-2">
                {loading ? (
                  ...
                  retiredTokens.map((token) => (
                    <div
                      key={token.tokenId}
                      className="border border-gray-800 rounded-lg p-3 bg-slate-900/60 text-sm text-gray-500"
                    >
                      <strong className="text-slate-400">
                        Token #{token.tokenId}
                      </strong>{" "}
                      from Factory {token.factoryId}
                      <p>
                        Retired On:{" "}
                        {new Date(
                          token.retirementTimestamp
                        ).toLocaleDateString()}
                      </p>
                    </div>
                  ))
                  ...
                  <p className="text-gray-500 text-center py-8">
                    You have not retired any credits yet.
                  </p>
                )}
              </div>
            </div>
          ...
          <div className="bg-[#21312E]/50 border border-gray-700/50 rounded-xl shadow-xl">
            <div className="p-6">
              ...
              <Notification
                ...
                onDismiss={() => setNotification({ message: null, type: null })}
              />
            </div>
          </div>
            ...
            <div className="p-6">
              ...
              <div className="space-y-3">
                {(buyerData?.marketPrices || []).map((price, index) => (
                  <div
                    key={index}
                    className="flex justify-between items-center text-sm p-3 bg-slate-800/50 rounded-lg"
                  >
                    <div>
