                ...
                >
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                    <span style={{ 
                      backgroundColor: 
                        tx.type === "mint" ? "#28a745" : 
                        tx.type === "transfer" ? "#17a2b8" : "#6c757d",
                      color: "white",
                      padding: "2px 6px",
                      borderRadius: "3px",
                      fontSize: "12px",
        ...
        <div className="card">
          ...
          <div style={{ marginTop: "15px" }}>
            <div style={{ marginBottom: "15px", display: "flex", gap: "10px", alignItems: "center" }}>
              <span>Total Verified: <strong>{tokens.length}</strong></span>
              <span>Active: <strong>{verifiedTokens.length}</strong></span>
              ...
              <table style={{ width: "100%", borderCollapse: "collapse" }}>
                <thead>
                  <tr style={{ backgroundColor: "#f8f9fa" }}>
                    ...
                    <th style={{ padding: "8px", border: "1px solid #ddd", textAlign: "left" }}>Verification Status</th>
        ...
        <div className="card">
          ...
          <div style={{ marginTop: "15px" }}>
            <div style={{ marginBottom: "15px" }}>
              <span>Total Facilities: <strong>{Object.keys(tokensByFactory).length}</strong></span>
            </div>
            
            <div style={{ maxHeight: "600px", overflowY: "auto" }}>
              {Object.entries(tokensByFactory).map(([factoryId, factoryTokens]) => {
                const activeTokens = factoryTokens.filter(t => !t.isRetired);
                const retiredTokens = factoryTokens.filter(t => t.isRetired);
                  ...
                  >
                    ...
                    <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(150px, 1fr))", gap: "10px", marginBottom: "10px" }}>
                      <div style={{ textAlign: "center", padding: "8px", backgroundColor: "white", borderRadius: "4px" }}>
                        <div style={{ fontSize: "18px", fontWeight: "bold", color: "#28a745" }}>{factoryTokens.length}</div>
                        <div style={{ fontSize: "12px", color: "#666" }}>Total Credits</div>
                      </div>
                      <div style={{ textAlign: "center", padding: "8px", backgroundColor: "white", borderRadius: "4px" }}>
                        <div style={{ fontSize: "18px", fontWeight: "bold", color: "#17a2b8" }}>{activeTokens.length}</div>
                        <div style={{ fontSize: "12px", color: "#666" }}>Active</div>
                      </div>
                      <div style={{ textAlign: "center", padding: "8px", backgroundColor: "white", borderRadius: "4px" }}>
                        <div style={{ fontSize: "18px", fontWeight: "bold", color: "#6c757d" }}>{retiredTokens.length}</div>
                        <div style={{ fontSize: "12px", color: "#666" }}>Retired</div>
                      </div>
                    </div>
                    
                    <div style={{ fontSize: "12px", color: "#666" }}>
                      First Production: {new Date(Math.min(...factoryTokens.map(t => new Date(t.creationTimestamp)))).toLocaleDateString()}
                      <br />
                      Last Production: {new Date(Math.max(...factoryTokens.map(t => new Date(t.creationTimestamp)))).toLocaleDateString()}
                    </div>
                  </div>
                );
              })}
          ...
          <div style={{
            ...
            <div style={{ marginTop: "15px" }}>
              <div style={{ padding: "10px", backgroundColor: "#d4edda", borderRadius: "4px", marginBottom: "15px" }}>
                <strong style={{ color: "#155724" }}>✓ VERIFIED ON BLOCKCHAIN</strong>
              </div>
              ...
              <div style={{ marginTop: "15px", padding: "10px", backgroundColor: "#f8f9fa", borderRadius: "4px" }}>
                <small>
                  ...
                  This token has been verified on the blockchain and represents a legitimate green hydrogen credit. 
                  All metadata is immutable and publicly verifiable.
                </small>
              </div>
            </div>
            <button
