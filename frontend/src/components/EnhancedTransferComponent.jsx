...
const EnhancedTransferComponent = ({
    ...
    if (identifier.length >= 3) {
      try {
        const response = await axios.get(
          `/api/blockchain/resolve-recipient/${identifier}`,
          {
            headers: {
  ...
  const handleTransfer = async (e) => {
    ...
    if (!recipientInfo) {
      onTransferError(
        "Recipient not found. Please enter a valid username, factory ID, or user ID"
      );
      return;
    }
    ...
    try {
      const response = await axios.post(
        "/api/blockchain/transfer-by-identifier",
        {
    ...
    <form onSubmit={handleTransfer} style={{ marginTop: "15px" }}>
      <div style={{ marginBottom: "15px" }}>
        ...
        <select
          ...
          onChange={(e) =>
            setTransferData((prev) => ({ ...prev, tokenId: e.target.value }))
          }
          style={{
            width: "100%",
            padding: "8px",
      ...
      {userTokens.filter((token) => !token.isRetired).length === 0 && (
        ...
        >
          No transferable tokens available. You can only transfer tokens you
          currently own.
        </p>
      )}
    </form>
